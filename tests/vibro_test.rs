...
fn test_vibro_control() {
    ...
    let expectations = [
        // 1. new() calls off() (sends 8 zero bytes)
        I2cTransaction::write(addr, vec![0u8; 8]),
        // 2. on(500ms, Medium)
        // Freq: 1000Hz (Default) = 0x000003E8 -> [E8, 03, 00, 00]
        // Duration: 500ms = 0x000001F4 -> [F4, 01, 00, 00]
        // Power: Medium = 35 = 0x23 -> [23, 00, 00, 00]
        I2cTransaction::write(
            addr,
            vec![
                0xE8, 0x03, 0x00, 0x00, 0xF4, 0x01, 0x00, 0x00, 0x23, 0x00, 0x00, 0x00,
            ],
        ),
        // 3. off() (sends 8 zero bytes)
        I2cTransaction::write(addr, vec![0u8; 8]),
    ];
